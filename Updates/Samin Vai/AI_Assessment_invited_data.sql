...
, applicantCTE AS (
	SELECT j.JP_ID, j.[Job Title], j.[Position Name], j.CP_ID, j.[Company Name], j.[Deadline Date], j.P_ID,
		CASE WHEN a.FinalSubmitted = 1 THEN 'YES' ELSE 'NO' END AS [Submitted]
...
j.JP_ID, j.[Job Title], j.[CP_ID], j.[Company Name], j.[Position Name], j.[Deadline Date], 
j.[Submitted]
FROM applicantCTE AS j
INNER JOIN bdjResumes..UserAccounts AS u ON u.accID = j.P_ID
WHERE CP_ID NOT IN (114174, 35450, 38918, 110412)
