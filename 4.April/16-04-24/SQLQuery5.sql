--SELECT TOP 5 * FROM bdjResumes..LOCATIONS
...
--SELECT TOP 100 * FROM bdjResumes..EXP

--SELECT TOP 5 * FROm [dbo].[DistrictAreaList]

--SELECT TOP 5 * FROm bdjResumes..vAddress

--SELECT TOp 5 * FROM bdjResumes..UserAddress
...
--	SELECT DISTINCT JP_ID, P_ID FROM bdjCorporate..DBO_JOB_INBOX WHERE JP_ID = 1236811
--)
--SELECT DISTINCT u.* FROM jobCTE AS j
--LEFT JOIN bdjResumes..UserAddress AS u ON u.P_ID = j.P_ID
...
--)
--select [NAME],[BIRTH],  DATEDIFF(year, p.[BIRTH], m.deadline) AS 'Age as of Deadline', [accPhone] as [MOBILE],[accEmail],[NID],[PRESENT_ADD],[PERMANENT_ADD],case when v.addresstype in (2) then FullAddress end as 'Present Location',case when v.addresstype in (1,3) then FullAddress end as 'Permanent Address', [L_Name] 'Home District'
--from maincte as m 
--left join bdjresumes.dbo.PERSONAL p on m.p_id = p.id
--left join bdjresumes.[dbo].[UserAccounts] ua on p.id = ua.accid
--left join bdjresumes.dbo.vAddress v on p.id = v.p_id
--left join bdjresumes.[dbo].[LOCATIONS] l on v.DistrictID =l.l_id



--, Addres_CTE AS (--SELECT J.P_ID, U1.Location AS PresentAddress, U2.Location AS PermanentAddress, U1.DistrictID AS D1 , U2.DistrictID AS D2, U1.ThanaID--FROM JOB_CTE J --LEFT JOIN bdjResumes..UserAddress U1 ON J.P_ID = U1.P_ID AND U1.AddressType IN (1,3)--LEFT JOIN bdjResumes..UserAddress U2 ON J.P_ID = U2.P_ID AND U2.AddressType IN (2,3)--), DIST_CTE AS (--SELECT A.P_ID--, REPLACE(REPLACE(' ' + ISNULL(A.PresentAddress, ' '), CHAR(10), ' '), CHAR(13), ' ') as PresentAddress --A.PresentAddress--, REPLACE(REPLACE(' ' + ISNULL(A.PermanentAddress, ' '), CHAR(10), ' '), CHAR(13), ' ') as PermanentAddress--A.PermanentAddress --, L.L_Name AS PresentDistrict, L1.L_Name AS PermanentDistrict, L2.L_Name AS Thana--, ROW_NUMBER() OVER(PARTITION BY A.P_ID ORDER BY A.PresentAddress, A.PermanentAddress DESC) AS [Row_Number]--FROM Addres_CTE A--LEFT JOIN bdjResumes..LOCATIONS L ON A.D1 = L.L_ID --LEFT JOIN bdjResumes..LOCATIONS L1 ON A.D2 = L1.L_ID --LEFT JOIN bdjResumes..LOCATIONS L2 ON A.ThanaID = L2.L_ID --300408--SELECT *, 
--ROW_NUMBER() OVER(PARTITION BY j.P_ID ORDER BY e3.[Exam Name: Graduation/Equivalent], e4.[Exam Name: Post-Graduation/Equivalent]) AS row_Number
--FROM jobCTE AS j
--	LEFT JOIN eduCTE1 AS e1 ON j.P_ID = e1.P_ID
--	LEFT JOIN eduCTE2 AS e2 ON j.P_ID = e2.P_ID
--	LEFT JOIN eduCTE3 AS e3 ON j.P_ID = e3.P_ID
