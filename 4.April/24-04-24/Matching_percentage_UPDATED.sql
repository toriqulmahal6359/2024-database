;WITH mainCTE as(
	...
	WHERE rp.UpdatedOn >= '01/01/2023' --AND rp.UpdatedOn <= '02/01/2023'
	AND jp.CP_ID >= 1234 and rp.applyid >= 212118561
	group by jp.CP_ID,c.NAME
	),main_cte2 as(	
	SELECT m.date,m.CP_ID,m.NAME--, j.Score
	,count(distinct jp.JP_ID) as T_JOB, count(distinct j.APPLYID)  as T_apply, m.T_shortlist, COUNT(DISTINCT v.P_ID) AS total_views,
	...
	INNER JOIN bdjCorporate..CompanyViewedCV AS v ON v.JP_ID = jp.JP_ID AND v.CP_ID = m.CP_ID
	where jp.JP_ID >=412884 and j.ApplyID>=8694811
	group by m.CP_ID,m.date,m.NAME,m.T_shortlist, m.[less_50%_shortlisted_applicant_count], m.[51% to 60%_shortlisted_applicant_count]
	...
	m1.total_views AS [total_cv_viewed],
	...
	m1.[less_50%_shortlisted_applicant_count],
	m3.[less_50%_shortlisted_applicant_percentage],
	m1.[51% to 60%_shortlisted_applicant_count],	m3.[51% to 60%_shortlisted_applicant_percentage],	m1.[61% to 70%_shortlisted_applicant_count],	m3.[61% to 70%_shortlisted_applicant_percentage],	m1.[71% to 80%_shortlisted_applicant_count],	m3.[71% to 80%_shortlisted_applicant_percentage],	m1.[81% to 90%_shortlisted_applicant_count],	m3.[81% to 90%_shortlisted_applicant_percentage],	m1.[91% to 100%_shortlisted_applicant_count],
	m3.[91% to 100%_shortlisted_applicant_percentage]
	--,m1.*,m2.*,m3.*
	--from mainCTE m
	--INNER JOIN 
	FROM main_cte2 m1 --on m.CP_ID = m1.CP_ID
	INNER JOIN main_cte3 m2 on m1.CP_ID = m2.CP_ID
	INNER JOIN main_cte4 m3 on m1.CP_ID = m3.CP_ID