...
CREATE PROCEDURE [dbo].[USP_INSERT_Blue_CV_v1](	  
	@p_id int = 0
		...
		BEGIN TRAN
			...
			IF @skillIDs <> '' And @accID > 0
				BEGIN
					...
					--VALUES (@p_id, @Name, @Birth_Date, @Gender, @mMobile, @Year_of_Exp, GETDATE(), GETDATE(), @GUID)
					SELECT accID, accFirstName + CASE WHEN (accLastName <> '' OR accLastName IS NOT NULL) THEN (' ' + accLastName) ELSE '' END, @Birth_Date, accGender, accPhone, @Year_of_Exp, GETDATE(), GETDATE(), @GUID
					from UserAccounts
					...
					SELECT P_ID, SkillID, SkilledByID, CASE WHEN ntvqfLevelID = 0 THEN NULL ELSE ntvqfLevelID END As ntvqfLevel, @GUID
					FROM Skill_CTE
			...
			IF @hasEducation = 1
				BEGIN
					...
					VALUES (@p_id, @Edu_Level, @Education, @Institute, @passing_year, @educationType,@BoardID, @GUID)
				END
