...
SELECT * FROM DBO_JOBPOSTINGS WHERE JP_ID = 1163930
...
WHERE j.PublishDate >= '05/01/2023 00:00:00' AND j.PublishDate <= '02/11/2024 00:00:00'
...
--SELECT * FROM DBO_JOBPOSTINGS WHERE JP_ID = 1225994

--SELECT COUNT(i.ApplyID) AS TotalApplications FROM DBO_JOBPOSTINGS AS j INNER JOIN DBO.DBO_JOB_INBOX AS i ON j.JP_ID = i.JP_ID
--WHERE j.JP_ID IN (1226927,
--1226936,
--1226802,
--1224835,
--1226531,
...
WHERE j.PublishDate >= '05-01-2023 00:00:00' --AND j.PublishDate <= @PublishDate
...
WHERE j.Drafted = 1
AND j.PostingDate >= '05/01/2023 00:00:00' --AND j.PublishDate <= '07/02/2024 03:20:00'
AND j.RegionalJob <> 4 --AND j.OnlineJob = 1
