--SELECT TOP 5 * FROM bdjCorporate..JobBillInfo

--SET STATISTICS TIME ON
;WITH Job_CTE AS ( --7056
...
, job_post_user as ( --5636 --7134
	...
	--GROUP BY J.JP_ID, UA.UserId, ua.accessgivenon
)

--SELECT * FROM job_post_user

, Job_User_CTE AS (--402  --7304 --7264
	SELECT J.JP_ID
...
--SELECT * FROM Job_post_user AS p
--LEFT JOIN Job_User_CTE AS u ON u.jp_id = p.Jp_id

, Job_Verification_CTE AS (--5608  --7304
...
--SELECT * FROM Job_post_user AS p
--LEFT JOIN Job_User_CTE AS u ON u.jp_id = p.Jp_id
--LEFT JOIN Job_Verification_CTE AS v ON v.Jp_id = u.jp_Id
...
--LEFT JOIN Job_User_CTE AS u ON u.jp_id = p.Jp_id
--LEFT JOIN Job_Verification_CTE AS v ON v.Jp_id = u.jp_Id
--LEFT JOIN JobPayment_CTE AS pc ON pc.JP_ID = u.Jp_Id
...
--LEFT JOIN Job_Verification_CTE AS v ON v.Jp_id = u.jp_Id
--LEFT JOIN JobPayment_CTE AS pc ON pc.JP_ID = u.Jp_Id
--LEFT JOIN CVOption_CTE AS cv ON cv.Jp_id = u.Jp_id

--SELECT * FROM Job_post_user AS p
--LEFT JOIN Job_User_CTE AS u ON u.jp_id = p.Jp_id
--LEFT JOIN Job_Verification_CTE AS v ON v.Jp_id = u.jp_Id
--LEFT JOIN JobPayment_CTE AS pc ON pc.JP_ID = u.Jp_Id
--LEFT JOIN Last_Activity_CTE a ON a.JP_ID = u.Jp_id
--LEFT JOIN CVOption_CTE AS cv ON cv.Jp_id = u.Jp_id
...
--SELECT * FROM JobList_CTE AS j
--LEFT JOIN Job_Verification_CTE AS v ON v.Jp_id = j.jp_Id
--LEFT JOIN JobPayment_CTE AS p ON p.JP_ID = j.Jp_Id
...
SELECT J.CP_ID, J.JP_ID, J.JobTitle, J.ProceedToPublishDate, J.PostingDate, J.Deadline, J.Verified, J.PublishDate
 ...
 , TestName, J.PrivilegedUsers, STRING_AGG(Cast(CUA.USERID as varchar(10))+' (-1)',',') AdminUsers--, J.TotalJob 
,j.ContactId
,j.UserId
FROM JobList_CTE J
...
, JP.JP_ID, J.ServiceType, J.Apply_Options, J.IsPublished, JobViewed, J.PostingDate, JV.SalesVerify, JV.AccountsVerify--, J.TotalJob
