	...
	DECLARE @id INT = 5327873 ; -- updated user 4141462   2332477  5327873 3434119 241028  6467114 2332477 241028
...
print(@UNStatus)
 ...
 BEGIN
	;WITH mainCTE as(
	   select CP.P_ID,cp.cpkstartdate,cp.cpkduration,cp.pkId, cp.IsActive, u.ApplicationBoostingLimit
	...
	WHERE s.duration = 1
	group by s.enddate, s.P_ID, s.IsActive, s.BoostingLimit, s.pkId
	),duration2 as(
 ...
 BEGIN
  ;WITH mainCTE as(
	   select CP.P_ID,cp.cpkstartdate,cp.cpkduration,cp.pkId, cp.IsActive, u.ApplicationBoostingLimit, u.ID
			from  bdjResumes.mnt.CandidatePackages CP
			INNER JOIN UserApplyLimit AS u ON u.P_ID = cp.P_ID
			where  cp.P_ID = @id  AND  CP.IsActive = 1
			--AND u.ApplicationBoostingLimit IS NOT NULL
