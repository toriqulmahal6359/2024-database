...
with Jobs as (select Distinct JP_IDFrom DBO_JOB_INBOX WHERE P_DATE >= '01/20/2024' AND P_DATE <= CONVERT(VARCHAR(100), GETDATE(), 101)    --'03/12/2024')Select J.JP_ID, COUNT(Distinct ApplyID) [Applicants],CASE WHEN (p.DeadLine >= CONVERT(VARCHAR(100), GETDATE(), 101)) THEN 'Live'	WHEN (p.DeadLine < CONVERT(VARCHAR(100), GETDATE(), 101)) THEN 'Expired' END AS [Status], PublishDate From Jobs JINNER JOIN DBO_JOB_INBOX I ON J.JP_ID = I.JP_IDINNER JOIN DBO_JOBPOSTINGS P ON P.JP_ID = J.JP_ID--where J.JP_ID in(1222791,1222801,1222802,1222803,1222805,1222021,1222022)Group by J.JP_ID, DeadLine, PublishDateOrder By J.JP_ID
...
SELECT j.JP_ID, COUNT(DISTINCT i.ApplyId) AS Applicants,
	CASE 
	...
	END AS [Status]
FROM [dbo].[DBO_JOB_INBOX] AS i
LEFT JOIN [dbo].[DBO_JOBPOSTINGS] AS j  ON i.JP_ID = j.JP_ID
WHERE j.PublishDate >= '01/20/2024 00:00:00' AND j.PublishDate <= '03/12/2024 13:00:00'
AND j.VERIFIED = 1 AND j.Drafted = 0 AND j.Closed = 0 AND j.OnlineJob = 1 AND j.RegionalJob <> 4
...
FROM DBO_JOBPOSTINGS J
WHERE J.Verified = 1 And J.Drafted = 0 And J.Closed = 0 And J.PublishDate >= '2023-05-01 00:00:00' And J.Deadline <= '2024-03-12 00:00:00'
AND RegionalJob <> 4 and OnlineJob = 1

{
  $and: [
   { "PublishDate": { $gte: ISODate("2023-05-01T00:00:00.000Z") } },
    { "DeadlineDate": { $lte: ISODate("2024-03-11T18:00:00.000Z") } },
    { "IsPaused": false },
	{ "ContentVerify": { $eq: 1 }},
	{ "IsDrafted": false }
  ]
...
--SELECT * FROM DBO_JOBPOSTINGS AS j
--LEFT JOIN DBO_BNG_JOBPOSTINGS AS bj ON j.JP_ID = bj.JP_ID
--WHERE j.JP_ID = 1184869