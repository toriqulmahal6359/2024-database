SELECT TOP 5 * FROm bdjCorporate..JobMatchFilters
SELECT DISTINCT FilterName FROM bdjCorporate..JobMatchFilters

SELECT CONVERT(DATE, j.PublishDate, 101) AS [date], COUNT(DISTINCT j.JP_ID) AS job, COUNT(DISTINCT j.CP_ID) AS [company] FROM bdjCorporate..DBO_JOBPOSTINGS AS j
LEFT JOIN bdjCorporate..JobMatchFilters AS m ON j.JP_ID = m.JP_ID
WHERE j.PublishDate >= '01/22/2024 00:00:00' and j.Drafted = 0 AND j.OnlineJob = 1 AND j.Verified = 1
-AND m.FilterName LIKE '%Location%' OR m.FilterName LIKE '%Salary%' 
GROUP BY CONVERT(DATE, j.PublishDate, 101)
HAVING COUNT(DISTINCT m.FilterName) = 3
