WITH jobCTE AS (
	...
	--	UNION
	SELECT DISTINCT j.JP_ID, CASE WHEN j.JobLang = 2 THEN bj.TITLE ELSE j.JobTitle END AS [Job Title], j.CP_ID, c.NAME AS [cp_name]
	--, ji.ApplyID, ji.P_ID
...
, subCTE AS (
	SELECT j.*, ji.ApplyID, ji.P_ID FROM jobCTE AS j
	INNER JOIN bdjCorporate..DBO_JOB_INBOX AS ji ON j.JP_ID = ji.JP_ID
	WHERE j.[Required Experience] IS NOT NULL
)
, countCTE AS (
