WITH jobCTE AS (
	...
	WHERE j.PublishDate >= '01/01/2024' AND j.VERIFIED = 1 AND j.OnlineJob = 1 and j.JP_ID >=277480 --and ji.ApplyID>=291878230
), countCTE AS (
	...
	FROM jobCTE AS j
	LEFT JOIN bdjCorporate..DBO_JOB_INBOX AS ji ON j.JP_ID = ji.JP_ID
	INNER JOIN bdjResumes..UserSummary AS v ON ji.P_ID = v.P_ID
	where [Required Experience] IS NOT NULL 
	GROUP BY j.JP_ID
)
SELECT CONVERT(VARCHAR(10), PublishDate, 101) AS [Publish Date],
	j.JP_ID, j.[Job Title]
