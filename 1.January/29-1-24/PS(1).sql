...
GO
	 ...
	 DECLARE
	 @PageNo int = 1
	,@uID int = 6056723        ----6098620  6056723 6152508   --4361771
	,@PageSize int = 200
	,@LanType char(2) = 'EN'
	,@FromDate varchar(10) =''
	,@SearchToDate varchar(15)=''
	,@SearchCompanyName nvarchar(240)= N''
	,@SearchIsViewed As int = NULL  -- 0 Not viewed, 1 viewed, 2 Video resume required, 3 hiered, 4 Offered, 5 Cotacted, 6 InterviewInvitation 
...
BEGIN	
	...
	SET @strSQL=@strSQL+ '	SELECT DISTINCT JI.ApplyID, JI.JP_ID, JI.P_ID, JI.Score, JI.P_Date, JI.Summary_Viewed, JI.ExpectedSalary, JI.EmpResponse, 0 as AssessmentNeeded, JI.Priority,JP.IsVDOResume' + CHAR(13)
	...
	SET @strSQL=@strSQL+ ' , CASE WHEN A.ApplyId IS NOT NULL THEN 1 ELSE 0 END [Status] ' + CHAR(13)
	SET @strSQL=@strSQL+ '	FROM DBO_JOB_INBOX JI ' + CHAR(13)
	SET @strSQL=@strSQL+ ' LEFT JOIN rp.ApplicantProcess A ON JI.ApplyID = A.ApplyId ' + CHAR(13)
	SET @strSQL=@strSQL+ '		LEFT JOIN dbo_JOBPOSTINGS JP ON JI.JP_ID = JP.JP_ID ' + CHAR(13)
   ...
   --Meharaz
	...
	--- Meharaz ------
	SET @strSQL=@strSQL+ '	, AR.ReasonId, AR.ReplyText , T.NoOfApply' + CHAR(13)
	...
	SET @strSQL=@strSQL+' , T.Status ' + CHAR(13) 
	...
	--SET @strSQL=@strSQL+ ' 	LEFT JOIN bdjResumes.[mnt].[ApplicationBoostingDetails] pb on UA.pkId = pb.pkId' + CHAR(13) 
